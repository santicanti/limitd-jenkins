...
echo "ls /opt/auth0/ reults"
ls /opt/auth0/
echo "ls /opt/auth0/limitd"
ls /opt/auth0/$NAME
cp /opt/auth0/$NAME/package/debian/$NAME-logs /etc/logrotate.d/
...
then
	cp /opt/auth0/$NAME/package/conf/limitd.conf.example /etc/limitd.conf
fi
